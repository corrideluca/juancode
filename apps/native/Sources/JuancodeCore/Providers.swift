...
public enum TerminalProfile {
    private static var profileRoot: String {
        (NSHomeDirectory() as NSString).appendingPathComponent(".corricode/terminal-profile")
    }

    private static var zshDir: String {
        (profileRoot as NSString).appendingPathComponent("zsh")
    }

    private static var bashRc: String {
        (profileRoot as NSString).appendingPathComponent("bashrc")
    }

    public static func shell() -> String {
        let override = ProcessInfo.processInfo.environment["CORRICODE_TERMINAL_SHELL"] ?? ""
        return override.isEmpty ? "/bin/zsh" : override
    }

    public static func startArgs() -> [String] {
        prepare()
        let name = (shell() as NSString).lastPathComponent
        if name == "bash" { return ["--rcfile", bashRc, "-i"] }
        return []
    }
    ...
    private static func prepare() {
        ...
        try? fm.createDirectory(atPath: profileRoot, withIntermediateDirectories: true)
        try? zshRc.write(toFile: (zshDir as NSString).appendingPathComponent(".zshrc"),
                         atomically: true, encoding: .utf8)
        try? bashProfile.write(toFile: bashRc, atomically: true, encoding: .utf8)
    }

    private static let zshRc = """
    # Generated by CorriCode for built-in Terminal sessions.
    ...
    PROMPT='%F{cyan}%n%f %F{blue}%1~%f${vcs_info_msg_0_} %# '
    RPROMPT='%F{244}%*%f'
    """

    private static let bashProfile = """
    # Generated by CorriCode for built-in Terminal sessions.
    ...
    __corricode_git_branch() {
      local branch
      branch=$(git symbolic-ref --quiet --short HEAD 2>/dev/null || git branch --show-current 2>/dev/null || git rev-parse --short HEAD 2>/dev/null) || return
      [ -n "$branch" ] || return
      printf ' (%s)' "$branch"
    }

    PS1='\\[\\e[36m\\]\\u\\[\\e[0m\\] \\[\\e[34m\\]\\W\\[\\e[0m\\]\\[\\e[33m\\]$(__corricode_git_branch)\\[\\e[0m\\] \\$ '
    """
}
...
public enum Providers {
    ...
    public static let terminal = ProviderSpec(
        ...
        label: "Terminal",
        pinsSessionId: false,
        startArgs: { _, _ in TerminalProfile.startArgs() },
        resumeArgs: { _, _ in [] }
    )
...
public struct DefaultBinaryResolver: BinaryResolver {
    ...
    public func command(for provider: ProviderId) -> String {
        ...
        case .terminal: return TerminalProfile.shell()
        }
    }
}
